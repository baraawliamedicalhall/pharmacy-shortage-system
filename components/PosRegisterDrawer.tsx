...
import { X, Printer, TrendingUp, DollarSign, Smartphone, Clock, Receipt, Pencil } from 'lucide-react'
...
interface PosRegisterDrawerProps {
  ...
  isAdmin?: boolean
  onEditSale?: (sale: ReceiptSaleData) => void
}
...
export function PosRegisterDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-950/50 backdrop-blur-xs flex justify-end" onClick={onClose}>
      <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-200" onClick={(e) => e.stopPropagation()}>
        {/* Drawer Header */}
            ...
            sales.map((sale) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-500">
                  <span>{sale.customerName || 'Walk-in'} &bull; {sale.paymentMethod}</span>
                ...
                <div className="pt-2 border-t border-slate-100 flex justify-end gap-1.5">
                  {isAdmin && onEditSale && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 transition-colors cursor-pointer"
                    >
